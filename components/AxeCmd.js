  ...
  try {
    ...
    if (htmlFiles.length > 0) {
      for (const file of htmlFiles) {
        ...
        try {
          driver = await driver.forBrowser("chrome")
            .withCapabilities(WebDriver.Capabilities.chrome())
            .setChromeOptions(new chrome.Options().headless()).build();
        } catch (de) {
          let deMessage = de.message;
          console.log(chalk.redBright("Chrome driver error: ", deMessage));
          socket.emit('infoWarningMsg', deMessage+'. (Now using Firefox for validation.)');
          driver = await driver.forBrowser("firefox")
            .withCapabilities(WebDriver.Capabilities.firefox())
            .setFirefoxOptions(new firefox.Options().headless()).build()
        }
        /*.forBrowser("chrome")
        .withCapabilities(WebDriver.Capabilities.chrome())
