...
const AxeCmdPuppeteer = async (req) => {
  ...
  if (htmlFiles.length > 0) {
    for (const file of htmlFiles) {
      ...
      if (url) {
        ...
        console.log('puppeteer opening the url: ', url)
        try {
          await page.goto(url, {
            waitUntil: 'load',
            // Remove the timeout
          ...
          const results = await new AxePuppeteer(page).disableRules(scanConfigObj.disableRules).analyze();
          debugIt && console.log('violation==', results.violations.length);
          ...
          socket.emit('infoWarningMsg', e.message);
          csvRowInfo.Score = '-';
          csvRowInfo.Message = e.message;
        }
        await browser.close();
    ...
    if (csvJsonArray.length > 0) {
      ...
      if (csvData) {
        ...
        socket.emit('a11yCsvReportMsg', `${API_EXPORT_AS_CSV}${csvFileName}`);
        console.log(
          chalk.bgGreenBright(
...
export default AxeCmdPuppeteer;
